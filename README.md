...
Both arms are **at or above nominal**, which is what a valid bound looks
like, and both are above it by a margin, which is what a *conservative* one
looks like. The 95% arm covered 2 876 admitted requests with zero violations.

The gap between the mean bound (2.60 s) and the mean realised latency
(0.72 s) is not a modelling failure to be tuned away. The dominant source of
uncertainty at admission time is the output length, which is genuinely
unknown and lognormally distributed in this workload; a bound that is honest
about that is wide. Validity and sharpness are separate properties, and only
